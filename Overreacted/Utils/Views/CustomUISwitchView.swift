...
class CustomSwitch: UIControl {
    ...
    @IBInspectable   var onTintColor: UIColor = UIColor(red: 144/255, green: 202/255, blue: 119/255, alpha: 1) {
        didSet {
            self.setupUI()
        }
    }
    
    @IBInspectable  var offTintColor: UIColor = UIColor.black {
        didSet {
            self.setupUI()
        }
    }
    ...
    @IBInspectable var thumbBorderWidth: CGFloat = .zero {
        didSet {
            self.thumbView.layer.borderWidth = self.thumbBorderWidth
        }
    }
    
    @IBInspectable var thumbBorderColor: UIColor = .tintColor {
        didSet {
    ...
    var thumbView = CustomThumbView(frame: .zero)
...
extension CustomSwitch {
    ...
    override func layoutSubviews() {
        ...
        if !self.isAnimating {
            ...
            self.onImageView.center = CGPoint(x: self.offPoint.x + self.thumbView.frame.size.width / 2, y: self.thumbView.center.y)
            self.offImageView.center = CGPoint(x: self.onPoint.x + self.thumbView.frame.size.width / 2, y: self.thumbView.center.y)
...
extension CustomSwitch {
    ...
    fileprivate func setOnOffImageFrame() {
        ...
        self.onImageView.center.x = !self.isOn ? 0 : self.offPoint.x + self.thumbView.frame.size.width / 2
        self.offImageView.center.x = self.isOn ? self.frame.width : self.onPoint.x + self.thumbView.frame.size.width / 2
        self.onImageView.alpha = self.isOn ? 1.0 : 0.0
        self.offImageView.alpha = self.isOn ? 0.0 : 1.0
    }
}
